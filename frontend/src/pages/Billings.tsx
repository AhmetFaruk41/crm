        ...
        <div className="card card-body lg:col-span-2 space-y-4">
          ...
          {data.billings.length > 0 && (
            <div>
              ...
              <table className="w-full text-sm border border-ink-200 rounded overflow-hidden">
                <thead>
                ...
                <tbody>
                  {data.billings.map((b: any, i: number) => (
                    <tr key={b.id} className="border-t border-ink-100">
                      ...
                      <td className="px-3 py-2 font-medium">{formatMoney(b.pay)}</td>
                      <td className="px-3 py-2">%{((Number(b.pay) / totals.total) * 100).toFixed(1)}</td>
                      <td className="px-3 py-2">{formatDate(b.create_date)}</td>
